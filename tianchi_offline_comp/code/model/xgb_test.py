...
##### output file
path_df_result     = "././data/output/xgb/res_xgb.csv"
path_df_result_tmp = "././data/output/xgb/res_xgb_tmp.csv"
...
'''Step 1: training for analysis of the best XGB model (parameters tuning)
        ...
        (3). tuning gamma 节点分裂时，只有分裂后损失函数的值下降了，才会分裂这个节点。
        (4). tuning subsample and colsample_bytree
        (5). tuning lambda & alpha 权重的L1正则化项 权重的L2正则化项
...
def f1_score(preds, dtrain):
    #preds是结果（概率值），dtrain是个带label的DMatrix
...
def CV_with_subsample(xg_param, xg_bst=None, cv_fold=5, train_np_ratio=1, valid_sub=1, train_sub=1, metric='f1-score', seed=None):
    ...
    for k in range(cv_fold):
        ...
        del(dvalid)
        del(bst_tmp)
        gc.collect()  # for saving my poor memory 清理内存
    # 返回预测结果和评分
