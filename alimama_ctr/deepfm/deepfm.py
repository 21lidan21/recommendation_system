...
def parse(feat_dict=None, df=None, has_label=False):
    ...
    if has_label:
        y = df['is_trade'].values.tolist()
        dfi.drop(['instance_id','is_trade'],axis=1, inplace=True)
    else:
        ids = dfi['instance_id'].values.tolist() # 预测样本的ids
...
train_file ="./data/query_0.csv"
test_file = "./data/query_1.csv"


IGNORE_FEATURES = [
    # 'id', 'target'
    "is_trade","instance_id", "context_id",
    "item_category_list","context_timestamp"
]
CATEGORITAL_FEATURES = [
    "predict_category_property",
    "user_id","user_gender_id","shop_id","item_id","user_occupation_id",
     "user_age_level", "user_star_level","shop_review_num_level","shop_star_level",
     "cate","item_brand_id", "item_city_id","context_page_id","day", "hour", "hour48",
      "query1", "query", "top1", "top2", "top3", "top4", "top5", "top10",
]
NUMERIC_FEATURES = [
     "item_property_list",
    # 浮点
     "item_price_level","item_sales_level","item_collected_level","item_pv_level","shop_review_positive_rate",
     "shop_score_service","shop_score_delivery","shop_score_description",
    #  先做numberic
     "same_cate","same_property",
    #  num
     "property_num","pred_cate_num", "pred_prop_num",
]
...
dfTrain = train_data[0:10000]
dfTest = test_data[0:3000]
...
for epoch in range(config.epochs):
    test_loss,opt = sess.run((loss, optimizer), feed_dict=feed_dict)
    print("prediction:")
    print("epoch: {0}, test loss: {1:.6f}".format(epoch, test_loss))
